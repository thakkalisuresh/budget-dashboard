...
async function ensureHistorySheet(sheetId, accessToken) {
  ...
  if (!exists) {
    ...
    const range = encodeURIComponent("'History'!A1:G1");
    await apiFetch(sheetId, `/values/${range}?valueInputOption=RAW`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ values: [['Timestamp', 'Action', 'Category', 'Vendor', 'Amount', 'Details', 'Non-monthly']] }),
    });
  }
...
export async function appendHistoryEntry(sheetId, accessToken, {
  ...
  try {
    ...
    const row = [timestamp, action, category, vendor, amount ?? '', details, nonMonthly ? 'Yes' : ''];
    const range = encodeURIComponent("'History'!A:G");
...
export async function fetchDetailRows(categoryName, accessToken, sheetId) {
  const config = SHEET_MAP[categoryName];
  if (!config) return [];
  const values = await fetchRaw(sheetId, config.sheet, accessToken);
  const result = [];
