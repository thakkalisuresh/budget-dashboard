...
/** Make a Drive file publicly readable so the Sheets API key can access it */
async function makePublicReadable(fileId, accessToken) {
  await fetch(
...
export function useMonths(accessToken) {
  ...
  /** Create a new month: copy template → make public → delete Months tab → update month columns → register */
  const createMonth = async (name) => {
    const newSheetId = await copyTemplate(name, accessToken);
    await makePublicReadable(newSheetId, accessToken);
  ...
  return { months, loading, createMonth, deleteMonth, reload: load };
}
