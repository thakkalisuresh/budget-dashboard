  ...
  return (
    <div className="animate-enter py-6 px-2" style={{ '--enter-delay': `${enterDelay}ms` }}>
      ...
      <p
        ...
        style={{ fontSize: 'clamp(2.5rem, 5vw, 3.5rem)', color: isNeg ? 'var(--color-danger)' : 'var(--color-text)' }}
      >
        <FormattedValue value={value} currencySymbol={currencySymbol} />
      </p>
      {subtext && (
...
function InlineStat({ title, value, subtext, onEdit, currencySymbol, enterDelay }) {
  const isNeg = value < 0;
  return (
    <div
      className="animate-enter rounded-xl px-5 py-4 flex items-center justify-between gap-6"
      style={{
        '--enter-delay': `${enterDelay}ms`,
        background: 'var(--color-surface)',
        border: '1px solid oklch(100% 0 0 / 8%)',
    ...
    >
      <div className="min-w-0">
        <p className="text-xs font-semibold tracking-wide truncate" style={{ color: 'var(--color-text-muted)' }}>{title}</p>
        {subtext && <p className="text-[11px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{subtext}</p>}
      </div>
